...
class CEEO_RS232():
    def __init__(self, divName, suffix='', myCSS=False, default_code=None):
        ...
        if myCSS:
            ...
            self.rs232div.innerHTML = RS232Style + \
                RS232HTML.format(num=self.suffix)
        ...
        document.getElementById(
            f'REPL{self.suffix}').onmousemove = self.movebar
        document.getElementById(
    ...
    async def on_connect(self, event):
        if self.uboard.connected:
            ...
            # False if you want to see existing code
            name = await self.uboard.board.connect(f'repl{self.suffix}', stop)
    ...
    async def on_upload(self, event):
        if self.uboard.connected:
            code = self.python.code
            path = window.prompt(
                'What do you want the filename to be on the processor?', self.list_files.value)
            if path:
    ...
    async def on_title(self, event):
        if self.uboard.connected:
            ...
            success = window.confirm(
                f'Did you want to change the name of your processor to {name}?')
            if success:
                if self.hub:
